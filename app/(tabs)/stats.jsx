...
export default function StatsScreen() {
  ...
  return (
    ...
    >
          ...
          ranked.map((row, i) => (
            <View key={row.id} style={[styles.rankRow, i === ranked.length - 1 && styles.rankRowLast]}>
              <View style={styles.medal}>
                <Text style={[typography.body, styles.rankEmoji]}>{emojiForRank(i)}</Text>
              </View>
      ...
      <Text style={[typography.heading, styles.section]}>{plus ? t('stats.monthGlance') : t('stats.thisMonth')}</Text>
      {months.map((monthStart) => (
        <View key={monthStart.toISOString()}>
          {plus ? (
        ...
        >
          <View style={styles.upsellTop}>
            <Text style={[typography.subheading, styles.upsellTitle]}>
              See your full history with Fajr+ - every month, since day one.
            </Text>
          </View>
...
function MonthGrid({ monthStart, today, habits, habitLogs, colors, spacing, radii }) {
